...
NEGATIVE_REGEX = [r"\b(log(in|out)?|sign(in|up)|register|account|my-account)\b", r"\b(anmelden|abmelden|registrieren|konto)\b", r"\b(iniciar-sesion|cerrar-sesion|crear-cuenta|cuenta)\b", r"\b(impressum|imprint|legal|disclaimer|compliance|datenschutz|privacy|terms|cookies?)\b", r"\b(agb|bedingungen|rechtliches|politica-de-privacidad|aviso-legal|terminos|condiciones)\b", r"\b(newsletter|subscribe|subscription|unsubscribe|boletin|suscripcion|darse-de-baja)\b", r"\b(jobs?|career(s)?|vacancies|internships?|apply|karriere|stellenangebote|bewerbung|praktikum|empleo|trabajo|vacantes|postulaciones|reclutamiento)\b", r"\b(basket|cart|checkout|shop|store|ecommerce|wishlist|warenkorb|kaufen|bestellen|einkaufen|carrito|tienda|comprar|pago|pedido)\b", r"\b(calculator|tool|search|filter|compare|rechner|suche|vergleich|calculadora|buscar|comparar|filtro)\b", r"\b(404|not-found|error|redirect|sitemap|robots|tracking|rss|weiterleitung|umleitung|redireccion|mapa-del-sitio|seguimiento)\b", r"\b(press[-_]release(s)?)\b"]
...
LINK_SCORE_MAP = {
    ...
    "core_business": {"patterns": [r"products", r"solutions", r"services", r"pipeline", r"research", r"innovation", r"investors?", r"investor[-_]relations"], "score": 15},
    ...
    "low": {"patterns": [r"blog", r"insights", r"articles", r"sectores", r"expertise", r"news", r"press", r"media"], "score": 3},
...
def _fetch_page_data_scrapfly(url: str, take_screenshot: bool = True):
    ...
    try:
        ...
        log("error", f"Exception in try block: {e}")
def fetch_html_with_playwright(url: str, retried: bool = False) -> Optional[str]:
    ...
    with sync_playwright() as p:
        try:
            ...
            log("info", "Hovering over navigation elements to reveal hidden links...")
            ...
            for link in nav_links:
                try:
                    ...
                    log("warn", f"Could not hover over a nav element: {e}")
                except Exception as e:
                    log("error", f"Exception in try block: {e}")
            log("info", "Finished hovering over navigation elements.")
...
def prepare_page_for_capture(page, max_ms=45000):
    ...
    page.evaluate("""() => { const step = 800; let y = 0; const sleep = ms => new Promise(r => setTimeout(r, ms)); (async () => { const maxScrolls = 50; let scrollCount = 0; while (y < document.body.scrollHeight && scrollCount < maxScrolls) { window.scrollBy(0, step); y += step; scrollCount++; await sleep(120); } window.scrollTo(0, 0); })(); }""")
    try:
...
def get_social_media_text(soup: BeautifulSoup, base_url: str) -> str:
    ...
    for platform, domain_pattern in social_domains.items():
        try:
            regex = re.compile(domain_pattern, re.IGNORECASE)
            candidate_tags = soup.find_all('a', href=regex)

            if not candidate_tags:
                log("info", f"No links found for {platform.capitalize()}.")
                continue

            good_links = []
            for tag in candidate_tags:
                href = tag.get('href', '')
                if 'intent' not in href and 'share' not in href and '/p/' not in href:
                    full_url = urljoin(base_url, href)
                    good_links.append(full_url)
            
            if not good_links:
                log("warn", f"Found {platform.capitalize()} links, but all were filtered out as irrelevant (e.g., share buttons).")
                continue

            # Remove duplicates and sort by length to find the most likely profile page
            unique_links = sorted(list(set(good_links)), key=len)
            best_url = unique_links[0]
            ...
            log("warn", f"Failed to process {platform.capitalize()} links: {e}")
...
def run_full_scan_stream(url: str, cache: dict):
    ...
    try:
        ...
        initial_links = []
        for a in initial_soup.find_all("a", href=True):
            href_raw = a.get("href")
            if not href_raw: continue
            href = _sanitize_href(href_raw)
            link_url = urljoin(initial_url, href)
            initial_links.append((link_url, a.get_text(strip=True)))
        ...
        if not discovered_links:
            log("warn", "Sitemap failed. Falling back to robust homepage scrape for link discovery.")
            if homepage_url == initial_url:
                homepage_html = initial_html # Reuse if we didn't pivot
            else:
                _, homepage_html = fetch_page_content_robustly(homepage_url)
            
            if not homepage_html: raise Exception("Could not fetch homepage HTML for link discovery.")
            ...
            for a in homepage_soup.find_all("a", href=True):
                ...
                if _is_same_root_domain(homepage_url, link_url):
                    discovered_links.append((link_url, a.get_text(strip=True)))

        
    if not discovered_links:
        log("warn", "Primary link discovery failed. Attempting fallback to homepage HTML scrape...")
        homepage_html = fetch_html(homepage_url)
        discovered_links = discover_links_from_homepage_html(homepage_html, homepage_url)

if not discovered_links: 
    log("warn", "Primary link discovery failed. Attempting fallback to homepage HTML scrape...")
    homepage_html = fetch_html(homepage_url)
    log("info", f"Fetched homepage HTML length: {len(homepage_html)}")
    discovered_links = discover_links_from_homepage_html(homepage_html, homepage_url)
    if not discovered_links:
        raise Exception("Fatal: Fallback also failed — no links discovered.")


        # --- PROCEED WITH THE REST OF THE SCAN ---
        try:
            homepage_screenshot_b64, homepage_html = fetch_page_content_robustly(homepage_url, take_screenshot=True)
            if not homepage_html: raise Exception("Could not fetch homepage content.")
        except Exception as e:
            log("error", f"Failed to fetch homepage URL: {e}")
            yield {'type': 'error', 'message': f'Failed to fetch homepage URL: {e}'}
            return
...
def discover_links_from_homepage_html(html: str, base_url: str):
