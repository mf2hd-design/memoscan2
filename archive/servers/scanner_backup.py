...
def discover_links_from_sitemap(homepage_url: str) -> Optional[List[Tuple[str, str]]]:
    ...
    try:
        response = httpx.get(sitemap_url, headers={"User-Agent": get_random_user_agent()}, timeout=20)
        response.raise_for_status()
        ...
        if root.tag.endswith('sitemapindex'):
            ...
            if best_sitemap_url:
                ...
                response = httpx.get(best_sitemap_url, headers={"User-Agent": get_random_user_agent()}, timeout=20)
                response.raise_for_status()
...
def run_full_scan_stream(url: str, cache: dict):
    ...
    try:
        ...
        if not discovered_links: raise Exception("Fatal: No links could be discovered.")
