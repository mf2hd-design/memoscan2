...
class CircuitBreaker:
    """
    Thread-safe circuit breaker per key_name.
    - Open after N consecutive failures; remain open for cooldown seconds
    - When open, primary (GPT-5.1) is skipped
    """
    ...
    def is_open(cls, key: str) -> bool:
        with cls._lock:
            ...
            if st["open_until"] <= cls._now():
                # Reset failures after cooldown expires
                if st["failures"] >= settings.CIRCUIT_BREAKER_THRESHOLD:
                    st["failures"] = 0
                return False
            return True
...
def _safe_responses_call(client, timeout_seconds: int = 60, max_retries: int = 2, **kwargs):
    """Safe Responses API call with timeout and retries."""
    last_error = None
    for retry in range(max_retries + 1):
        if retry > 0:
            time.sleep(min(2 ** retry, 8))  # Exponential backoff
        with concurrent.futures.ThreadPoolExecutor(max_workers=1) as ex:
            ...
            try:
                ...
                fut.cancel()
                last_error = TimeoutError(f"Responses call timed out after {timeout_seconds}s")
            except Exception as e:
                # Don't retry on non-timeout errors
                raise e
    raise last_error or Exception("Unexpected responses call error")
