"""
Brand House LangGraph workflow.
Strategic brand positioning and rebrand recommendation.
"""
...
from ..nodes.brand_audit_nodes import node_identify_competitors, node_scrape_competitors, node_scrape_news_mentions
from ..nodes.brand_house_nodes import node_research_brand_positioning, node_analyze_brand_house
...
def should_skip_to_format(state: Dict[str, Any]) -> str:
    ...
    if state.get("cache_hit"):
        ...
        return "format"
    return "research_positioning"


async def create_brand_house_workflow():
    """
    Create and compile the Brand House workflow.

    Workflow steps:
    1. Initialize
    2. Cache check
    3. Research brand positioning
    4. Identify competitors (reused from Brand Audit)
    5. Scrape competitors (reused from Brand Audit)
    6. Scrape news (reused from Brand Audit)
    7. GPT-5.1 analysis
    ...
    workflow.add_node("research_positioning", node_research_brand_positioning)
    ...
    workflow.add_conditional_edges(
        ...
        {
            "format": "format",
            "research_positioning": "research_positioning"
        }
    )
    workflow.add_edge("research_positioning", "identify_competitors")
